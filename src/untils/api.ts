...
export async function callApi(option: IOptionCallApi = initOptionCallApi): Promise<IResponse> {

  const response = await ((option.method !== Emethod.GET && fetch(
    `${option.url}/${option.path}`,
    {
