...
export class AppTable extends React.Component<PropsAppTable> {
    ...
    constructor(props: Readonly<PropsAppTable>) {
        ...
        this._groupColums = this.props.columns.filter(
            column => !column?.pid?.trim()).map((column) => (
                {
                    ...column,
                    childrens: this.props.columns.filter(item => item?.pid === column.key)
                }
            )
            );
    }
    renderThearParent(column: (ITableColumn & { childrens: ITableColumn[] }), index: number) {
        const { classes, sort, columsSort } = this.props;
        return (<TableCell
            className={`${column.childrens.length > 0 && classes.topHeaderHasChildren} ${classes.topHeader} ${column.key}`}
            key={`thear-${index}`}
    ...
    renderThear(column: (ITableColumn & { childrens: ITableColumn[] }), index: number) {
        const { classes, sort, columsSort, order, orderBy } = this.props;
        if (column?.childrens?.length > 0) {
            return column.childrens?.map((children, index) => (<TableCell
                key={`thear-${index}`}
                style={children.styleThear && children.styleThear}
                className={
                    `${children.key} ${classes.topHeaderGroup}`
                }
            >{
                    ...
                    (
                        ...
                        <TableSortLabel
                            direction={order}
                        >
                            {(children.renderThear && children.renderThear(children) || children.label)}
                        </TableSortLabel>
                    )
                    ||
                    (children.renderThear && children.renderThear(children) || children.label)
        ...
        return (
            <TableCell
                ...
                style={column.styleThear && column.styleThear}
                className={
                    `${column.key} ${this._pColumns.length > 0 ? classes.topHeaderGroup : classes.topHeaderNotGroup}`
                }
            >{
                    (
    ...
    renderCell(row: any, column: (ITableColumn & { childrens: ITableColumn[] }), index: number) {
        const converRow = row as any
        if (column?.childrens?.length > 0) {
            return column.childrens?.map((children) => {
                const check: IRowSpan[] = this._rowSpans.filter(row => row.key === column.key);
                ...
                const data = (
                    ...
                    >
                        {
                            (children.renderCell && children.renderCell(cell)) || converRow[children.key]
                        }
                    </TableCell>)) || null
                if (this._columSpan === 0)
                    this._columSpan = (children.columSpanCell && (children.columSpanCell - 1)) || 0;
                else {
                    this._columSpan--;
                }
                if (rowSpan === null) {
                    ...
                    this._rowSpans = this._rowSpans.map((row) => {
                        if (row.key === rowSpan.key) {
                            const d = {
                                ...row,
                                break: row.break === 0 ? ((children.rowSpanCell && (children.rowSpanCell - 1)) || 0) : (row.break - 1)
                            }
                            return d;
                        } else {
                            return row
            ...
            const data = (this._columSpan === 0 && (rowSpan === null || rowSpan.break === 0) && (
                (<TableCell
                ...
                >
                    {
                        (column.renderCell && column.renderCell(cell)) || converRow[column.key]
                    }
                </TableCell>)) || null
            )

            if (this._columSpan === 0)
                this._columSpan = (column.columSpanCell && (column.columSpanCell - 1)) || 0;
            else {
                this._columSpan--;
            }
            ...
            if (rowSpan === null) {
                ...
                this._rowSpans = this._rowSpans.map((row) => {
                    if (row.key === rowSpan.key) {
                        return {
                            ...row,
                            break: row.break === 0 ? ((column.rowSpanCell && (column.rowSpanCell - 1)) || 0) : (row.break - 1)
                        }
                    } else {
                        return row
                    }
    ...
    handleChangeRowsPerPage = (perPage: string) => {
        ...
        return this.props.onChangeRowsPerPage && this.props.onChangeRowsPerPage(per)
    }
    generateData() {
            ...
            <>
                <TableContainer component={Paper}>
                    <Table className={classes.table} aria-label="simple">
                        <TableHead>
                            {
                                ...
                                (<TableRow style={this.props.rowStyle}>
                                    {
                                        this._groupColums.map((column: (ITableColumn & { childrens: ITableColumn[] }), index: number) => this.renderThearParent(column, index))
                                    }
                                </TableRow>)
                            }
                            ...
                            {
                                (this._pColumns.length > 0) && (<TableRow style={this.props.rowStyle}>
                                    {
                                        this._groupColums.filter(group => group?.childrens.length > 0)
...

                            {
                                (this._pColumns.length === 0) && (<TableRow style={this.props.rowStyle}>
                                    {
                                        this._groupColums.filter(group => group?.childrens).map((column: (ITableColumn & { childrens: ITableColumn[] }), index: number) => this.renderThear(column, index))
                                    }
                                </TableRow>)
                            }
                        ...
                        <TableBody>
                            {data && this.generateData().map((item: any, index: number) => (
                                <TableRow
                                    key={index} onClick={() => onClickRow && onClickRow(item)}
                                    style={this.props.rowStyle}
                                >
                                    {
                                        this._groupColums
                                            .map(
                                                (column) => this.renderCell(item, column, index)
                                            )
                                    }
                                </TableRow>
                            ))}
